...
function renderLineChart(data,yLabel,xLabel="tempo(ms)"){
    return (
        <div style={{margin:20}}>
            <LineChart width={800} height={400} data={data} margin={{bottom:20,top:20,left:50}}>
                <Line type="monotone" dataKey="y" stroke="#8884d8" />
                <CartesianGrid stroke="#ccc" />
                <XAxis dataKey="x" label={{value:xLabel,position:"bottom"}}/>
                <YAxis domain={[]}>
                    <Label value={yLabel} position="insideBottomLeft" offset={-30} angle={-90}/>
                </YAxis>
                <Tooltip />
            </LineChart>
        </div>
...
function Graphs()
{
    ...
    return (
        <Container>
            ...
            {renderLineChart(measurement?formatData(measurement.times,measurement.temperatures):[],"temperatura(ºC)")}
            {renderLineChart(measurement?formatData(measurement.times,measurement.speeds):[],"velocidade(m/s)")}
            {renderLineChart(measurement?formatData(measurement.times,measurement.positions):[],"posição(m)")}
            {renderLineChart(measurement?formatData(measurement.times,measurement.magnetic_fields_avg):[],"média do campo magnético")}
        </Container>
    );
}
